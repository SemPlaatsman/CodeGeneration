...
# Feature: Get Account by IBAN
  Scenario: Successfully get an account by IBAN
    # Given the API is running
    And there is an account with IBAN "NL06INHO0000000001"
    When a request is made to GET /accounts/123456789
    Then the response status should be 200
    And the response body should contain the account object with IBAN "NL06INHO0000000001"

  Scenario: Attempt to get a non-existent account by IBAN
    # Given the API is running
    And there is no account with IBAN "NL06INHO0000000001"
    When a request is made to GET /accounts/NL06INHO0000000001
...
# Feature: Delete Account
  Scenario: Successfully delete an account
    # Given the API is running
    And there is an account with IBAN "NL06INHO0000000001"
    When a request is made to DELETE /accounts/NL06INHO0000000001
